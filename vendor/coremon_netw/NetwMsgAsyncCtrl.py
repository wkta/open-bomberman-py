...
from coremon_netw.EnvoiAsynchrone import EnvoiAsynchrone


class AsyncNetwMessageCtrl(EventReceiver):
    mem_timestamp = None
    ...
    def on_async_request(self, ev):
        ...
        #     EnvoiAsynchrone2.PORT = self.port
        #     EnvoiAsynchrone2.HOST_STR = self.host

        print('[AsyncMessagingCtrl] processing async request {}'.format(ev))
        id_req = ev.num
        msg_ressource = ev.msg

        print('appel constr EnvoiAsynchrone avec {}  ||  {}'.format(self.host, msg_ressource))
        thread = EnvoiAsynchrone(self.host, msg_ressource) #ev.data, self.__class__.mem_timestamp)
